...
const UserDataForm = () => {
    ...
    const form = useForm({
        ...
        // mode: 'onBlur'               //when Focus on field, error msg show & when enter continue appear
        mode: 'onTouched'               //when Focus on field, error msg show & when enter input data msg dissappear
        // mode: 'onChange'             //when enter input data check validation for each input
        // mode: 'all'                  //combination of onBlur & onChange mode
    })
    ...
    return (
        <div>
            ...
            <form onSubmit={handleSubmit(onFormSubmit, onError)} noValidate >
                ...
                <input type="text" id="SecondaryNumber" maxLength={10} {...register("mobile.1", {
                ...
                <div>
                    ...
                    <div>
                        ...
                        <button type="button" onClick={() => append({ number: '' })}>Add New number</button>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div>
                            ...
                            data.map((field2, index) => {
                                return (
                                    <div className="form-control" key={field2.id}>
                                        ...
                                        {
                                            index > 0 && (<button type="button" onClick={() => removeData(index)}>Remove</button>)
                                        }
                        ...
                        <button type="button" onClick={() => appendData({ hobby: '' })}>Add New Hobby</button>
                    </div>
                </div>

                <button disabled={!isDirty || isSubmitting || !isValid}>Submit</button>
